...
long long mapping(long long f, long long s) { return f + s; }

long long composition(long long f1, long long f2) { return f1 + f2; }

long long id() { return 0; }

typedef LazySegtree<long long, op, e, long long, mapping, composition, id>
    lazySegtree;
...
int main() {
    ...
    int N, K;
    cin >> N >> K;
    vector<lazySegtree> trees(K + 1, lazySegtree(N + 1));
    vector<vector<long long>> dp(N + 1, vector<long long>(K + 1));
    for (int i = 1; i <= N; ++i) {
        long long A, B;
        cin >> A >> B;
        dp[i][0] = dp[i - 1][0] + A;
        trees[0].Set(i, dp[i][0]);
        for (int j = 1; j <= K; ++j) {
            trees[j - 1].Apply(0, i, B);
            dp[i][j] = max(dp[i - 1][j] + A, trees[j - 1].Prod(0, i));
            trees[j].Set(i, dp[i][j]);
        }
    }
    long long ans = 0;
    for (int i = 0; i <= K; ++i) {
