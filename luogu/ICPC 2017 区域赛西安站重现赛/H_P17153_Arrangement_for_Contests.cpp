...
int mapping(int f, int s) { return s + f; }
...
int main() {
    ...
    while (T--) {
        ...
        LazySegtree<int, op, e, int, mapping, composition, id> lazySegTree(a);
        long long ans = 0;
        for (int i = 0; i + K - 1 < N; ++i) {
            int mn = lazySegTree.Prod(i, i + K);
            if (mn == 0) {
                continue;
            }
            ans += mn;
            lazySegTree.Apply(i, i + K, -mn);
        }
        cout << ans << '\n';
    }
